...
UnitTest.asynctest('tinymce.lists.browser.IndentTest', function () {
    ...
    suite.test('Indent single LI in OL', function (editor) {
        editor.getBody().innerHTML = LegacyUnit.trimBrs('<ol>' +
            '<li>a</li>' +
            '</ol>');
        editor.focus();
    ...
    suite.test('Indent single LI in OL and retain OLs list style in the new OL', function (editor) {
        editor.getBody().innerHTML = LegacyUnit.trimBrs('<ol style="list-style-type: lower-alpha;">' +
            '<li>a</li>' +
            '<li>b</li>' +
            '</ol>');
        editor.focus();
        LegacyUnit.setSelection(editor, 'li:nth-child(2)', 0);
        LegacyUnit.execCommand(editor, 'Indent');
        LegacyUnit.equal(editor.getContent(), '<ol style="list-style-type: lower-alpha;">' +
            '<li>a' +
    ...
    suite.test('Indent second LI to same level as nested LI', function (editor) {
        editor.getBody().innerHTML = LegacyUnit.trimBrs('<ul>' +
            '<li>a</li>' +
            '<li>b' +
            '<ul>' +
            '<li>c</li>' +
    ...
    TinyLoader.setup(function (editor, onSuccess, onFailure) {
        ...
        indent: false,
        entities: 'raw',
        valid_elements: 'li[style|class|data-custom],ol[style|class|data-custom],' +
...
//# sourceMappingURL=IndentTest.js.map
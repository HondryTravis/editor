...
UnitTest.asynctest('browser.tinymce.plugins.image.ImagePluginTest', function () {
    ...
    suite.test('Default image dialog on empty editor', function (editor) {
        ...
        LegacyUnit.equal(cleanHtml(editor.getContent()), '<p><img src="src" alt="alt" width="200" height="100" /></p>');
    });
    suite.test('Image dialog image_dimensions: false', function (editor) {
        ...
        LegacyUnit.equal(cleanHtml(editor.getContent()), '<p><img src="src" alt="alt" /></p>');
        teardown(editor);
    });
    if (Env.ceFalse) {
        suite.test('All image dialog ui options on empty editor', function (editor) {
            ...
            LegacyUnit.equal(cleanHtml(editor.getContent()), '<figure class="image">' +
        ...
        suite.test('All image dialog ui options on empty editor (old IE)', function (editor) {
            ...
            LegacyUnit.equal(cleanHtml(editor.getContent()), '<p><img class="class1" src="src" alt="alt" width="100" height="200" /></p>');
        });
    }
    suite.test('Image recognizes relative src url and prepends relative image_prepend_url setting.', function (editor) {
        ...
        win.find('form')[0].submit();
        win.close();
        LegacyUnit.equal(cleanHtml(editor.getContent()), '<p><img src="' + editor.settings.image_prepend_url + 'src" alt="alt" /></p>');
    });
    ...
    suite.test('Advanced image dialog border option on editor with content', function (editor) {
        ...
        LegacyUnit.equal(cleanHtml(editor.getContent()), '<p>a<img style="border-width: 10px; border-style: dashed;" src="src" alt="alt" /></p>');
    });
    suite.test('Advanced image dialog margin space options on empty editor', function (editor) {
        ...
        LegacyUnit.equal(cleanHtml(editor.getContent()), '<p><img style="margin: 10px;" src="src" alt="alt" /></p>');
    });
    suite.test('Advanced image dialog border style only options on empty editor', function (editor) {
        ...
        LegacyUnit.equal(cleanHtml(editor.getContent()), '<p><img style="border-width: 10px; border-style: solid;" src="src" alt="alt" /></p>');
    });
    ...
    suite.test('Advanced image dialog overriden border style options on empty editor', function (editor) {
        ...
        LegacyUnit.equal(cleanHtml(editor.getContent()), '<p><img style="border-width: 10px;" src="src" alt="alt" /></p>');
    });
...
//# sourceMappingURL=ImagePluginTest.js.map
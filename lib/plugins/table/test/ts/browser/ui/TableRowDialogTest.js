...
UnitTest.asynctest('browser.tinymce.plugins.table.TableRowDialogTest', function () {
    ...
    suite.test('Table row properties dialog (update all)', function (editor) {
        ...
        LegacyUnit.equal(cleanTableHtml(editor.getContent()), '<table><thead><tr style="height: 10px; text-align: right;"><td>X</td></tr></thead></table>');
        closeTopMostWindow(editor);
    });
    suite.test('Caption should always stay the firstChild of the table (see TINY-1167)', function (editor) {
        ...
        LegacyUnit.equal(cleanTableHtml(editor.getContent()), '<table><caption>CAPTION</caption><thead><tr><td>X</td></tr></thead><tbody><tr><td>Y</td></tr></tbody></table>');
        closeTopMostWindow(editor);
    });
    suite.test('Table row properties dialog update multiple rows', function (editor) {
        editor.getBody().innerHTML = ('<table>' +
            '<tbody>' +
        ...
        LegacyUnit.equal(cleanTableHtml(editor.getContent()), ('<table>' +
            '<tbody>' +
            ...
            '</tr>' +
            '</tbody>' +
            '</table>'), 'Width should be retained height should be changed');
        closeTopMostWindow(editor);
    });
...
//# sourceMappingURL=TableRowDialogTest.js.map
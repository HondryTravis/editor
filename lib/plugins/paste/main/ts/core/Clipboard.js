...
import { Futures, Future, Arr } from '@ephox/katamari';
import { Image, navigator } from '@ephox/dom-globals';
/**
...
var isClipboardEvent = function (event) { return event.type === 'paste'; };
var readBlobsAsDataUris = function (items) {
    return Futures.mapM(items, function (item) {
...
var getImagesFromDataTransfer = function (dataTransfer) {
    var items = dataTransfer.items ? Arr.map(Arr.from(dataTransfer.items), function (item) { return item.getAsFile(); }) : [];
    var files = dataTransfer.files ? Arr.from(dataTransfer.files) : [];
    var images = Arr.filter(items.length > 0 ? items : files, function (file) { return /^image\/(jpeg|png|gif|bmp)$/.test(file.type); });
    return images;
};
/**
    ...
    if (editor.settings.paste_data_images && dataTransfer) {
        ...
        if (images.length > 0) {
            ...
            readBlobsAsDataUris(images).get(function (blobResults) {
                ...
                Arr.each(blobResults, function (result) {

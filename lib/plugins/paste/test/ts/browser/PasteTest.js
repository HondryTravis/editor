...
UnitTest.asynctest('tinymce.plugins.paste.browser.ImagePasteTest', function () {
    ...
    suite.test('Paste Word links', function (editor) {
        ...
        LegacyUnit.equal(editor.getContent(), ('<p>' +
    ...
    suite.test('paste nested (UL) word list', function (editor) {
        editor.setContent('');
        editor.execCommand('mceInsertClipboardContent', false, {
            content: ('<p class=MsoListParagraphCxSpFirst style=\'text-indent:-18.0pt;mso-list:l0 level1 lfo1\'>' +
        ...
        LegacyUnit.equal(editor.getContent(), '<ul>' +
            '<li>a' +
            '<ul>' +
            '<li>b' +
            '<ul>' +
    ...
    suite.test('paste nested (OL) word list', function (editor) {
        ...
        LegacyUnit.equal(editor.getContent(), '<ol>' +
            '<li>a' +
            '<ol>' +
            '<li>b' +
            '<ol>' +
    ...
    suite.test('Paste list start index', function (editor) {
        ...
        editor.setContent('');
        editor.execCommand('mceInsertClipboardContent', false, {
            content: ('<p class=MsoListParagraphCxSpMiddle style="text-indent:-18.0pt;mso-list:l0 level1 lfo1">' +
    ...
    suite.test('paste invalid content with spans on page', function (editor) {
        var startingContent = '<p>123 testing <span id="x">span later in document</span></p>', insertedContent = '<ul><li>u</li><li>l</li></ul>';
    ...
    if (Env.webkit) {
        ...
        suite.test('paste webkit remove runtime styles (color) in the same (color) (named)', function (editor) {
            ...
            editor.execCommand('mceInsertClipboardContent', false, {
                content: ('<span style="color:#ff0000; text-indent: 10px">a</span>' +
        ...
        suite.test('paste webkit remove runtime styles (color) in the same (color) (hex)', function (editor) {
            ...
            LegacyUnit.setSelection(editor, 'p', 0, 'p', 4);
            editor.execCommand('mceInsertClipboardContent', false, {
                content: ('<span style="color:red; text-indent: 10px">a</span>' +
...
//# sourceMappingURL=PasteTest.js.map
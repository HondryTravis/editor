 ...
 */
import { AddEventsBehaviour, AlloyEvents, AlloyTriggers, Behaviour, Button, Container, Disabling, Form, Highlighting, Keying, Memento, NativeEvents, Representing } from '@ephox/alloy';
...
var sketch = function (rawSpec) {
    ...
    var memForm = Memento.record(Form.sketch(function (parts) {
        return {
        ...
        behaviours: Behaviour.derive([
            ...
            AddEventsBehaviour.config(wrapperAdhocEvents, [
                AlloyEvents.run(NativeEvents.touchstart(), function (wrapper, simulatedEvent) {
                    var event = simulatedEvent.event();
                    spec.state.dialogSwipeState.set(SwipingModel.init(event.touches[0].clientX));
                }),
                AlloyEvents.run(NativeEvents.touchmove(), function (wrapper, simulatedEvent) {
                    var event = simulatedEvent.event();
                    spec.state.dialogSwipeState.on(function (state) {
                        simulatedEvent.event().prevent();
                        spec.state.dialogSwipeState.set(SwipingModel.move(state, event.raw().touches[0].clientX));
                    });
                }),
...
export { sketch };
//# sourceMappingURL=SerialisedDialog.js.map
...
UnitTest.asynctest('browser.tinymce.core.html.DomParserTest', function () {
    ...
    suite.test('Parse element', function () {
        ...
        LegacyUnit.deepEqual(root.firstChild.attributes, [{ name: 'title', value: 'title' },
            { name: 'class', value: 'class' }], 'Element attributes');
    ...
    suite.test('Whitespace', function () {
        ...
        LegacyUnit.equal(serializer.serialize(root), '<ul><li>Item 1<ul><li>Indented</li></ul></li></ul>', 'Whitespace around and inside blocks (ul/li)');
        ...
        LegacyUnit.equal(serializer.serialize(root), '<div><img src="file.gif" data-mce-src="file.gif" /></div>', 'Whitespace where SaxParser will produce multiple whitespace nodes');
        LegacyUnit.deepEqual(countNodes(root), { body: 1, div: 1, img: 1 }, 'Whitespace where SaxParser will produce multiple whitespace nodes (count)');
    });
    ...
    suite.test('Parse invalid contents', function () {
        ...
        LegacyUnit.equal(serializer.serialize(root), '<p class="a">a</p><p class="b">b</p><p class="c">c</p><p class="a">d</p>', 'Two P in P, no nodes before/after');
        ...
        LegacyUnit.equal(serializer.serialize(root), '<p class="a">a<strong>b<span>c<em>d</em></span></strong></p><p class="b">e</p>' +
            '<p class="a"><strong><span><em>f</em>g</span>h</strong>i</p>', 'P in P wrapped in inline elements');
        LegacyUnit.deepEqual(countNodes(root), { 'body': 1, 'p': 3, '#text': 9, 'strong': 2, 'span': 2, 'em': 2 }, 'P in P wrapped in inline elements (count)');
        parser = DomParser({}, schema);
        root = parser.parse('<p class="a">a<p class="b">b<p class="c">c</p>d</p>e</p>');
        LegacyUnit.equal(serializer.serialize(root), '<p class="a">a</p><p class="b">b</p><p class="c">c</p><p class="b">d</p><p class="a">e</p>', 'P in P in P with text before/after');
    ...
    suite.test('Parse invalid contents with attribute filters', function () {
        ...
        var root = parser.parse('<p class="y">a<p class="y">123</p>b</p>');
        LegacyUnit.equal(serializer.serialize(root), '<p class="x">a</p><p class="x">123</p><p class="x">b</p>', 'P should have class x');
    });
    ...
    suite.test('Remove empty elements', function () {
        var parser;
        var schema = Schema({ valid_elements: 'span,-a,img' });
        parser = DomParser({}, schema);
        ...
        LegacyUnit.equal(serializer.serialize(root), '<span></span><a href="#"><img src="about:blank" /></a>', 'Leave elements with img in it');
    });
    suite.test('Self closing list elements', function () {
        ...
        LegacyUnit.equal(serializer.serialize(root), '<ul><li>1</li><li><strong>2</strong></li><li><em><strong>3</strong></em></li></ul>', 'Split out LI elements in LI elements.');
    });
    suite.test('Remove redundant br elements', function () {
        var parser, root;
        var schema = Schema();
        parser = DomParser({ remove_trailing_brs: true }, schema);
        root = parser.parse('<p>a<br></p>' +
            '<p>a<br>b<br></p>' +
            '<p>a<br><br></p><p>a<br><span data-mce-type="bookmark"></span><br></p>' +
            '<p>a<span data-mce-type="bookmark"></span><br></p>');
        LegacyUnit.equal(serializer.serialize(root), '<p>a</p><p>a<br />b</p><p>a<br /><br /></p><p>a<br /><br /></p><p>a</p>', 'Remove traling br elements.');
    });
    ...
    suite.test('Remove br in invalid parent bug', function () {
        var parser, root;
        var schema = Schema({ valid_elements: 'br' });
        parser = DomParser({ remove_trailing_brs: true }, schema);
    ...
    suite.test('Forced root blocks', function () {
        ...
        root = parser.parse('<!-- a -->' +
            ...
            '<b>g</b>' +
            'h');
        LegacyUnit.equal(serializer.serialize(root), '<!-- a --><p>b<strong>c</strong></p><p>d</p><p>e</p><p>f<strong>g</strong>h</p>', 'Mixed text nodes, inline elements and blocks.');
    });
    suite.test('Forced root blocks attrs', function () {
        ...
        LegacyUnit.equal(serializer.serialize(root), '<!-- a -->' +
            ...
            '<p>d</p>' +
            '<p>e</p>' +
            '<p class="class1">f<strong>g</strong>h</p>', 'Mixed text nodes, inline elements and blocks.');
    });
    suite.test('Parse html4 lists into html5 lists', function () {
        ...
        LegacyUnit.equal(serializer.serialize(root), '<ul><li style="list-style-type: none"><ul><li>a</li></ul></li></ul><ul><li>a<ul><li>b</li></ul></li></ul>');
    });
    ...
    suite.test('Parse contents with html5 anchors and allow_html_in_named_anchor: false', function () {
        var parser, root;
        var schema = Schema({ schema: 'html5' });
        parser = DomParser({ allow_html_in_named_anchor: false }, schema);
    ...
    suite.test('Parse contents with html5 self closing datalist options', function () {
        ...
        LegacyUnit.equal(serializer.serialize(root), '<datalist><option label="a1" value="b1"></option><option label="a2" value="b2"></option>' +
    ...
    suite.test('Invalid text blocks within a li', function () {
        var parser, root;
        var schema = Schema({ schema: 'html5', valid_children: '-li[p]' });
        parser = DomParser({}, schema);
    ...
    suite.test('Valid classes', function () {
        var parser, root;
        var schema = Schema({ valid_classes: 'classA classB' });
        parser = DomParser({}, schema);
    ...
    suite.test('Valid classes multiple elements', function () {
        var parser, root;
        var schema = Schema({ valid_classes: { '*': 'classA classB', 'strong': 'classC' } });
        parser = DomParser({}, schema);
    ...
    suite.test('Bug #7543 removes whitespace between bogus elements before a block', function () {
        ...
        LegacyUnit.equal(serializer.serialize(DomParser().parse('<div><b data-mce-bogus="1">a</b> <b data-mce-bogus="1">b</b><p>c</p></div>')), '<div>a b<p>c</p></div>');
    });
    suite.test('Bug #7582 removes whitespace between bogus elements before a block', function () {
        var serializer = Serializer();
        LegacyUnit.equal(serializer.serialize(DomParser().parse('<div>1 <span data-mce-bogus="1">2</span><div>3</div></div>')), '<div>1 2<div>3</div></div>');
    });
    suite.test('do not replace starting linebreak with space', function () {
        var serializer = Serializer();
        LegacyUnit.equal(serializer.serialize(DomParser().parse('<p>a<br />\nb</p>')), '<p>a<br />b</p>');
    });
    suite.test('getAttributeFilters/getNodeFilters', function () {
        var parser = DomParser();
        var cb1 = function (nodes, name, args) { };
        var cb2 = function (nodes, name, args) { };
        parser.addAttributeFilter('attr', cb1);
        parser.addNodeFilter('node', cb2);
        var attrFilters = parser.getAttributeFilters();
        var nodeFilters = parser.getNodeFilters();
...
//# sourceMappingURL=DomParserTest.js.map
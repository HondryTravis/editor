...
UnitTest.asynctest('browser.tinymce.core.content.InsertContentCommandTest', function (success, failure) {
    ...
    suite.test('mceInsertContent - text inside empty p', function (editor) {
        ...
        LegacyUnit.equal(editor.getBody().innerHTML.toLowerCase().replace(/^<br>/, ''), '<p>abc</p>'); // Opera inserts a BR at the beginning of contents if the P is empty
        rng = normalizeRng(editor.selection.getRng(true));
        ok(rng.collapsed);
        LegacyUnit.equal(rng.startContainer.nodeName, 'P');
        LegacyUnit.equal(rng.startOffset, 1);
    ...
    suite.test('mceInsertContent - legacy content', function (editor) {
        ...
        LegacyUnit.equal(editor.getContent(), '<p><span style="text-decoration: line-through;">strike</span><span style="font-size: 300%;">font</span></p>');
    });
    ...
    suite.test('mceInsertContent - invalid insertion with spans on page', function (editor) {
        var startingContent = '<p>123 testing <em>span later in document</em></p>', insertedContent = '<ul><li>u</li><li>l</li></ul>';
...
//# sourceMappingURL=InsertContentCommandTest.js.map
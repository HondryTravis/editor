...
UnitTest.asynctest('browser.tinymce.core.dom.DomUtilsTest', function () {
    var DOM = DOMUtils(document, { keep_values: true, schema: Schema() });
    ...
    suite.test('parseStyle', function () {
        ...
        dom = DOMUtils(document, { hex_colors: true, keep_values: true, url_converter: function (u) {
                return 'X' + u + 'Y';
            } });
        LegacyUnit.equal(dom.serializeStyle(dom.parseStyle('border: 1px solid red; color: green')), 'border: 1px solid red; color: green;');
        LegacyUnit.equal(dom.serializeStyle(dom.parseStyle('border: 1px solid rgb(0, 255, 255); color: green')), 'border: 1px solid #00ffff; color: green;');
        LegacyUnit.equal(dom.serializeStyle(dom.parseStyle('border-top: 1px solid red; border-left: 1px solid red; border-bottom: 1px solid red; border-right: 1px solid red;')), 'border: 1px solid red;');
        LegacyUnit.equal(dom.serializeStyle(dom.parseStyle('border-width: 1pt 1pt 1pt 1pt; border-style: none none none none; border-color: black black black black;')), 'border: 1pt none black;');
        LegacyUnit.equal(dom.serializeStyle(dom.parseStyle('border-width: 1pt 4pt 2pt 3pt; border-style: solid dashed dotted none; border-color: black red green blue;')), 'border-width: 1pt 4pt 2pt 3pt; border-style: solid dashed dotted none; border-color: black red green blue;');
        LegacyUnit.equal(dom.serializeStyle(dom.parseStyle('background: transparent url(test.gif);')), 'background: transparent url(\'Xtest.gifY\');');
        LegacyUnit.equal(dom.serializeStyle(dom.parseStyle('background: transparent url(http://www.site.com/test.gif?a=1&b=2);')), 'background: transparent url(\'Xhttp://www.site.com/test.gif?a=1&b=2Y\');');
    ...
    suite.test('createHTML', function () {
        LegacyUnit.equal(DOM.createHTML('span', { id: 'id1', class: 'abc 123' }, 'content <b>abc</b>'), '<span id="id1" class="abc 123">content <b>abc</b></span>');
    ...
    suite.test('setGetAttrib', function () {
        ...
        LegacyUnit.equal(DOM.getAttrib('test', 'class'), '123');
        LegacyUnit.equal(DOM.getAttrib('test', 'title'), 'abc');
        dom = DOMUtils(document, { keep_values: true, url_converter: function (u, n) {
                return '&<>"' + u + '&<>"' + n;
            } });
    ...
    suite.test('getAttribs', function () {
        ...
        LegacyUnit.equal(check(DOM.getAttribs('test2'), 'id,type,name,value,disabled,readonly,checked'), true, 'Expected attributed: type,name,disabled,readonly,checked');
        DOM.remove('test');
    });
    ...
    suite.test('getOuterHTML', function () {
        ...
        LegacyUnit.equal(Tools.trim(DOM.get('test').innerHTML).toLowerCase().replace(/>\s+</g, '><').replace(/\"/g, ''), '<div id=test2>123</div><div id=test3>abc</div>');
    ...
    suite.test('split', function () {
        ...
        DOM.split(parent, point);
        LegacyUnit.equal(HtmlUtils.cleanHtml(DOM.get('test').innerHTML), '<ul><li>first line<br><ul><li><span>second</span> <span>line</span></li></ul></li><li>third line<br></li></ul>');
        DOM.remove('test');
    });
    ...
    suite.test('isEmpty without defined schema', function () {
        DOM.add(document.body, 'div', { id: 'test' }, '');
        var domUtils = DOMUtils(document);
        DOM.setHTML('test', '<hr>');
    ...
    suite.test('isEmpty with list of elements considered non-empty without schema', function () {
        var domWithoutSchema = DOMUtils(document, { keep_values: true });
...
//# sourceMappingURL=DomUtilsTest.js.map
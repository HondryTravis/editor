...
UnitTest.asynctest('browser.tinymce.core.dom.SelectionTest', function () {
    ...
    suite.test('getSelectedBlocks with collapsed selection between elements', function (editor) {
    ...
    suite.test('getNode', function (editor) {
        ...
        editor.selection.setRng(rng);
        LegacyUnit.equalDom(editor.selection.getNode(), editor.dom.get('s1'), 'Detect selection ends immediately after node at start of paragraph.');
        rng = editor.dom.createRng();
        ...
        editor.selection.setRng(rng);
        LegacyUnit.equalDom(editor.selection.getNode(), editor.dom.get('s2'), 'Detect selection immediately surrounds node in middle of paragraph.');
        rng = editor.dom.createRng();
        ...
        editor.selection.setRng(rng);
        LegacyUnit.equalDom(editor.selection.getNode(), editor.dom.get('s3'), 'Detect selection starts immediately before node at end of paragraph.');
        rng = editor.dom.createRng();
        rng.setStart(editor.dom.get('s2').previousSibling, editor.dom.get('s2').previousSibling.length);
        rng.setEnd(editor.dom.get('s3').lastChild, editor.dom.get('s3').lastChild.length);
        editor.selection.setRng(rng);
        LegacyUnit.equalDom(editor.selection.getNode(), editor.dom.get('p1'), 'Detect selection wrapping multiple nodes does not collapse.');
    });

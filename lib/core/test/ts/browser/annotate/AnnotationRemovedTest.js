...
UnitTest.asynctest('browser.tinymce.core.annotate.AnnotationRemovedTest', function (success, failure) {
    ModernTheme();
    TinyLoader.setup(function (editor, onSuccess, onFailure) {
        var tinyApis = TinyApis(editor);
        var sSetupData = GeneralSteps.sequence([
        ...
        var outside1 = { path: [0, 0], offset: 'Th'.length };
        var inside1 = { path: [0, 1, 0], offset: 'i'.length };
        var inside3 = { path: [2, 1, 0], offset: 'i'.length };
        ...
        var sTestGetAndRemove = GeneralSteps.sequence([
            tinyApis.sSetSelection(outside1.path, outside1.offset, outside1.path, outside1.offset),
            Waiter.sTryUntil('Nothing active (outside1)', tinyApis.sAssertContentPresence({
                '.mce-annotation': 3
            }), 100, 1000),
            Logger.t('There should be two alpha annotations', sAssertGetAll(editor, {
                'id-one': 1,
                'id-two': 1
            }, 'alpha')),
            Logger.t('There should be one beta annotation', sAssertGetAll(editor, {
                'id-three': 1
            }, 'beta')),
            Step.sync(function () {
            ...
            Step.wait(1000),
            Waiter.sTryUntil('removed alpha, but was not inside alpha', tinyApis.sAssertContentPresence({
                '.mce-annotation': 3
            }), 100, 1000),
            Logger.t('There should be still be two alpha annotations (because remove only works if you are inside)', sAssertGetAll(editor, {
                'id-one': 1,
                'id-two': 1
            }, 'alpha')),
            Logger.t('There should still be one beta annotation', sAssertGetAll(editor, {
                'id-three': 1
            }, 'beta')),
            ...
            Waiter.sTryUntil('removed beta', tinyApis.sAssertContentPresence({
                '.mce-annotation': 2
            }), 100, 1000),
            Logger.t('There should be still be two alpha annotations (because cursor was inside beta)', sAssertGetAll(editor, {
                'id-one': 1,
                'id-two': 1
            }, 'alpha')),
            Logger.t('There should be no beta annotations', sAssertGetAll(editor, {}, 'beta')),
            tinyApis.sSetSelection(inside1.path, inside1.offset, inside1.path, inside1.offset),
            Step.sync(function () {
                editor.annotator.remove('alpha');
            }),
            Waiter.sTryUntil('removed alpha, and was inside alpha', tinyApis.sAssertContentPresence({
                '.mce-annotation': 1
            }), 100, 1000),
            Logger.t('There should now be just one alpha annotation (second one was removed)', sAssertGetAll(editor, {
                'id-two': 1
            }, 'alpha')),
            Logger.t('There should be no beta annotations', sAssertGetAll(editor, {}, 'beta')),
        ]);
...
//# sourceMappingURL=AnnotationRemovedTest.js.map
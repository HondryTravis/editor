...
UnitTest.asynctest('browser.tinymce.core.annotate.AnnotationChangedTest', function (success, failure) {
    ModernTheme();
    var changes = Cell([]);
    var sAssertChanges = function (message, expected) { return Logger.t(message, 
    // Use a chain so that changes.get() can be evaluated at run-time.
    Chain.asStep({}, [
        Chain.mapper(function (_) {
            return changes.get();
        }),
        Chain.op(function (cs) {
            Assertions.assertEq('Checking changes', expected, cs);
        })
    ])); };
    var sClearChanges = Step.sync(function () {
        changes.set([]);
    });
    TinyLoader.setup(function (editor, onSuccess, onFailure) {
        var tinyApis = TinyApis(editor);
        var sTestAnnotationEvents = function (label, start, soffset, expected) {
            return GeneralSteps.sequence([
                tinyApis.sSetSelection(start, soffset, start, soffset),
                Waiter.sTryUntil(label, sAssertChanges('sTestAnnotationEvents.sAssertChanges', expected), 10, 1000),
            ]);
        };
        var sTestChanges = GeneralSteps.sequence([
            ...
            Waiter.sTryUntil('Waiting for no changes', sAssertChanges('Should be no changes', []), 10, 1000),
            sTestAnnotationEvents('No annotation at cursor', [0, 0], 'Th'.length, [
                { state: false, name: 'delta', uid: null },
                { state: false, name: 'gamma', uid: null }
            ]),
            sTestAnnotationEvents('At annotation alpha, id = id-one', [0, 1, 0], 'i'.length, [
                { state: false, name: 'delta', uid: null },
                { state: false, name: 'gamma', uid: null },
                { state: true, name: 'alpha', uid: 'id-one' }
            ]),
            sTestAnnotationEvents('At annotation alpha, id = id-two', [1, 1, 0], 'hi'.length, [
                { state: false, name: 'delta', uid: null },
                { state: false, name: 'gamma', uid: null },
                { state: true, name: 'alpha', uid: 'id-one' },
            ...
            Step.wait(400),
            Waiter.sTryUntil('Moving selection within the same marker (alpha id-two) ... shoud not fire change', sAssertChanges('checking changes', [
                { state: false, name: 'delta', uid: null },
                { state: false, name: 'gamma', uid: null },
                { state: true, name: 'alpha', uid: 'id-one' },
                { state: true, name: 'alpha', uid: 'id-two' }
            ]), 10, 1000),
            sTestAnnotationEvents('Outside annotations again', [1, 2], ' the '.length, [
                { state: false, name: 'delta', uid: null },
                { state: false, name: 'gamma', uid: null },
                { state: true, name: 'alpha', uid: 'id-one' },
                { state: true, name: 'alpha', uid: 'id-two' },
            ...
            sTestAnnotationEvents('Inside annotation beta, id = id-three', [2, 1, 0], 'i'.length, [
                { state: false, name: 'delta', uid: null },
                { state: false, name: 'gamma', uid: null },
                { state: true, name: 'alpha', uid: 'id-one' },
                { state: true, name: 'alpha', uid: 'id-two' },
            ...
            Waiter.sTryUntil('Moving selection outside all annotations. Should fire null', sAssertChanges('checking changes', [
                { state: false, name: 'delta', uid: null },
                { state: false, name: 'gamma', uid: null },
                { state: true, name: 'alpha', uid: 'id-one' },
                { state: true, name: 'alpha', uid: 'id-two' },
            ...
            // Give it time to throttle a node change.
            Step.wait(400),
            Waiter.sTryUntil('Moving selection outside all annotations (again). Should NOT fire null because it already has', sAssertChanges('checking changes', [
                { state: false, name: 'delta', uid: null },
                { state: false, name: 'gamma', uid: null },
                { state: true, name: 'alpha', uid: 'id-one' },
                { state: true, name: 'alpha', uid: 'id-two' },
            ...
            // Give it time to throttle a node change.
            Step.wait(400),
            Waiter.sTryUntil('Moving selection inside delta (which is inside gamma)', sAssertChanges('checking changes', [
            ...
            // Give it time to throttle a node change.
            Step.wait(400),
            Waiter.sTryUntil('Moving selection inside just gamma (but not delta)', sAssertChanges('checking changes', [
                { state: true, name: 'delta', uid: 'id-five' },
                { state: true, name: 'gamma', uid: 'id-four' },
                { state: false, name: 'delta', uid: null }
            ]), 10, 1000),
        ...
        setup: function (ed) {
            ed.on('init', function () {
                ...
                var listener = function (state, name, obj) {
                    ...
                    if (state === false) {
                        ...
                        var uid = obj.uid, nodes = obj.nodes;
                        // In this test, gamma markers span multiple nodes
                        if (name === 'gamma') {
                            Assertions.assertEq('Gamma annotations must have 2 nodes', 2, nodes.length);
                        }
                        assertMarker(ed, { uid: uid, name: name }, nodes);
                    }
                    changes.set(changes.get().concat([
                        { uid: state ? obj.uid : null, name: name, state: state }
                    ]));
                };
...
//# sourceMappingURL=AnnotationChangedTest.js.map
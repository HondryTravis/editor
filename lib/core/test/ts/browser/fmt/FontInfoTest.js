...
UnitTest.asynctest('browser.tinymce.core.fmt.FontInfoTest', function () {
    ...
    var assertComputedFontProp = function (fontProp, html, path, expected) {
        ...
        LegacyUnit.equal(actual, expected, 'Doesn\'t match the expected computed runtime style');
    ...
    var assertSpecificFontProp = function (fontProp, html, path, expected) {
        ...
        var elm = Hierarchy.follow(Element.fromDom(div), path).getOrDie('oh no! ' + path.toString() + '  path was bad');
        var actual = fontGetProp(div, elm.dom());
        LegacyUnit.equal(actual, expected, 'Doesn\'t match the expected specific element style');
        div.parentNode.removeChild(div);
    };
    ...
    suite.test('getFontFamily', function () {
        ...
        assertSpecificFontProp('fontFamily', '<span style="font-family: Arial, Helvetica, Verdana"><mark></mark></span>', [0, 0], 'Arial,Helvetica,Verdana');
        assertSpecificFontProp('fontFamily', '<span style="font-family: Arial, Verdana"><span><mark></mark></span>', [0, 0, 0], 'Arial,Verdana');
        assertSpecificFontProp('fontFamily', '<span style="font-family: Arial, Helvetica, Verdana"><span><mark></mark></span></span>', [0, 0, 0], 'Arial,Helvetica,Verdana');
    });
    ...
    suite.asyncTest('getFontFamily should return a string when run on element in removed iframe', function (_, done, die) {
        ...
        iframe.addEventListener('load', function () {
            ...
            var firstChildElement = iframe.contentDocument.body.firstChild;
            iframe.parentNode.removeChild(iframe);
            try {
                var fontFamily = FontInfo.getFontFamily(body, firstChildElement);
    ...
    suite.test('should not throw error when passed in element without parent', function () {
        var rootDiv = document.createElement('div');
        var element = document.createElement('p');
        var actual = FontInfo.getFontSize(rootDiv, element);
...
//# sourceMappingURL=FontInfoTest.js.map
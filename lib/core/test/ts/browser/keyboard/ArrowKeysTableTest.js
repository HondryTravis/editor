...
UnitTest.asynctest('browser.tinymce.core.keyboard.ArrowKeysTableTest', function (success, failure) {
    var browser = PlatformDetection.detect().browser;
    ModernTheme();
    var table = function (html) { return ApproxStructure.fromHtml('<table><tbody><tr><td>' + html + '</td></tr></tbody></table>'); };
    var block = ApproxStructure.fromHtml('<p><br></p>');
    var caret = function (type) {
        return ApproxStructure.fromHtml("<p data-mce-caret=\"" + type + "\" data-mce-bogus=\"all\"><br data-mce-bogus=\"1\"></p>");
    };
    var visualCaret = function (before) {
        var caretClass = before ? 'mce-visual-caret-before' : 'mce-visual-caret';
        return ApproxStructure.fromHtml("<div class=\"mce-visual-caret " + caretClass + "\" data-mce-bogus=\"all\"></div>");
    };
    var caretBefore = Fun.curry(caret, 'before');
    var caretAfter = Fun.curry(caret, 'after');
    var visualCaretBefore = Fun.curry(visualCaret, true);
    var visualCaretAfter = Fun.curry(visualCaret, false);
    var buildBody = function (children) { return ApproxStructure.build(function (s, str, arr) { return s.element('body', { children: children }); }); };
...
//# sourceMappingURL=ArrowKeysTableTest.js.map
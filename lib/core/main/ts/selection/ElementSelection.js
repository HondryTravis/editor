...
var getEndpointElement = function (root, rng, start, real, resolve) {
    ...
    return Option.from(container).map(SugarElement.fromDom).map(function (elm) {
        return !real || !rng.collapsed ? Traverse.child(elm, resolve(elm, offset)).getOr(elm) : elm;
    }).bind(function (elm) { return SugarNode.isElement(elm) ? Option.some(elm) : Traverse.parent(elm); }).map(function (elm) { return elm.dom(); }).getOr(root);
};

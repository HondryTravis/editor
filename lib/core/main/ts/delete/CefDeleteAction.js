...
var isCompoundElement = function (node) { return ElementType.isTableCell(Element.fromDom(node)) || ElementType.isListItem(Element.fromDom(node)); };
...
var isDeleteFromCefDifferentBlocks = function (root, forward, from, to) {
    var inSameBlock = function (elm) { return ElementType.isInline(Element.fromDom(elm)) && !CaretUtils.isInSameBlock(from, to, root); };
    return CaretUtils.getRelativeCefElm(!forward, from).fold(function () { return CaretUtils.getRelativeCefElm(forward, to).fold(Fun.constant(false), inSameBlock); }, inSameBlock);
};
...
var read = function (root, forward, rng) {
    var normalizedRange = CaretUtils.normalizeRange(forward ? 1 : -1, root, rng);
    ...
    if (forward === false && CaretUtils.isAfterContentEditableFalse(from)) {
        ...
        return findPreviousBr(rootElement, from).map(function (br) { return DeleteAction.remove(br.getNode()); });
    }
    else if (forward && CaretUtils.isAfterContentEditableFalse(from) && isBeforeBr(rootElement, from)) {
        return findNextBr(rootElement, from).map(function (br) { return DeleteAction.remove(br.getNode()); });
    }
    else {
...
export { read };
//# sourceMappingURL=CefDeleteAction.js.map
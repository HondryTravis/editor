...
var isBlock = NodeType.matchStyleValues('display', 'block table');
var isFloated = NodeType.matchStyleValues('float', 'left right');
var isValidElementCaretCandidate = Predicate.and(isElement, isCaretCandidate, Fun.not(isFloated));
var isNotPre = Fun.not(NodeType.matchStyleValues('white-space', 'pre pre-line pre-wrap'));
var isText = NodeType.isText;
var isBr = NodeType.isBr;
var nodeIndex = DOMUtils.nodeIndex;
var resolveIndex = RangeNodes.getNode;
var createRange = function (doc) { return 'createRange' in doc ? doc.createRange() : DOMUtils.DOM.createRng(); };
var isWhiteSpace = function (chr) { return chr && /[\r\n\t ]/.test(chr); };
var isRange = function (rng) { return !!rng.setStart && !!rng.setEnd; };
...
var isZeroRect = function (r) { return r.left === 0 && r.right === 0 && r.top === 0 && r.bottom === 0; };
...
var collapseAndInflateWidth = function (clientRect, toStart) {
    var newClientRect = GeomClientRect.collapse(clientRect, toStart);
 ...
 */
export function CaretPosition(container, offset, clientRects) {
    ...
    var getNode = function (before) { return resolveIndex(container, before ? offset - 1 : offset); };
    return {
        /**
...
(function (CaretPosition) {
     ...
     */
    CaretPosition.fromRangeEnd = function (range) { return CaretPosition(range.endContainer, range.endOffset); };
    /**
     ...
     */
    CaretPosition.after = function (node) { return CaretPosition(node.parentNode, nodeIndex(node) + 1); };
    /**
     ...
     * @return {tinymce.caret.CaretPosition} Caret position from the node.
     */
    CaretPosition.before = function (node) { return CaretPosition(node.parentNode, nodeIndex(node)); };
    ...
    CaretPosition.isAtStart = function (pos) { return pos ? pos.isAtStart() : false; };
    CaretPosition.isAtEnd = function (pos) { return pos ? pos.isAtEnd() : false; };
    CaretPosition.isTextPosition = function (pos) { return pos ? NodeType.isText(pos.container()) : false; };
    CaretPosition.isElementPosition = function (pos) { return CaretPosition.isTextPosition(pos) === false; };

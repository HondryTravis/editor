...
var table = (function () {
    ...
    var getHtml = function (cols, rows, colorMap, hasColorPicker) {
      ...
      var id = global$4.DOM.uniqueId('mcearia');
      var getColorCellHtml = function (color, title) {
        var isNoColor = color === 'transparent';
        return '<td class="mce-grid-cell' + (isNoColor ? ' mce-colorbtn-trans' : '') + '">' + '<div id="' + id + '-' + count++ + '"' + ' data-mce-color="' + (color ? color : '') + '"' + ' role="option"' + ' tabIndex="-1"' + ' style="' + (color ? 'background-color: ' + color : '') + '"' + ' title="' + global$5.translate(title) + '">' + (isNoColor ? '&#215;' : '') + '</div>' + '</td>';
      };
      colors = TextColor.mapColors(colorMap);
      colors.push({
        text: global$5.translate('No color'),
        color: 'transparent'
      });
      ...
      if (hasColorPicker) {
        html += '<tr>' + '<td colspan="' + cols + '" class="mce-custom-color-btn">' + '<div id="' + id + '-c" class="mce-widget mce-btn mce-btn-small mce-btn-flat" ' + 'role="button" tabindex="-1" aria-labelledby="' + id + '-c" style="width: 100%">' + '<button type="button" role="presentation" tabindex="-1">' + global$5.translate('Custom...') + '</button>' + '</div>' + '</td>' + '</tr>';
        html += '<tr>';
        for (x = 0; x < cols; x++) {
    ...
    var getColorPickerCallback$1 = function (editor) {
      return editor.getParam('color_picker_callback', null);
    };
    var hasColorPicker = function (editor) {
      return typeof getColorPickerCallback$1(editor) === 'function';
    };
    var Settings = {
      ...
      getColorPickerCallback: getColorPickerCallback$1,
    ...
    var addMenuItems = function (editor, selections) {
      ...
      editor.addMenuItem('insertrowbefore', {
        text: 'Insert row before',
        onclick: cmd('mceTableInsertRowBefore')
      });
      editor.addMenuItem('insertrowafter', {
        text: 'Insert row after',
        onclick: cmd('mceTableInsertRowAfter')
      });
      editor.addMenuItem('insertcolbefore', {
        text: 'Insert column before',
        onclick: cmd('mceTableInsertColBefore')
      });
      editor.addMenuItem('insertcolafter', {
        text: 'Insert column after',
        onclick: cmd('mceTableInsertColAfter')
      });
      editor.addMenuItem('deleterow', {
        text: 'Delete row',
        onclick: cmd('mceTableDeleteRow')
      });
      editor.addMenuItem('deletecol', {
        text: 'Delete column',
        onclick: cmd('mceTableDeleteCol')
      });
